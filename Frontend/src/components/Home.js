...
const Home = () => {
  ...
  // VIEW COURSE
   ...
   const fetchmovie=()=>{
      axios.get("http://localhost:5000/api/viewmovies")
      .then((response)=>{
   ...
   const deletemovie=(id)=>{
    console.log(id)
    const data={_id:id}
    axios.post("http://localhost:5000/api/deletemovie",data)
    .then((response)=>{
        console.log(response.data);
                    ...
                    <TableBody>
                        {
                            movies.map((value, index) => (
                                <StyledTableRow key={index}>
                                    ...
                                    <Link  to={`/edit/${value._id}`} className='links'><Button variant='contained' color='primary'>Update</Button></Link>
                                    </StyledTableCell>
                                    <StyledTableCell align='right'>
